...
```{r, fig.cap = "Largest companies in the world by market value"}
...
company %<>% 
  ...
  mutate(across(c(Marketcap, Operating_cash_flow, Levered_free_cash_flow),
...
company_plot %<>% 
  mutate(`Earnings-to-price` = 1/PER,
         ...
         `Book-to-price` = 1/PBR,
         Name_trimmed = clean_name(Name)) %>%
  arrange(desc(Industry), 
          !is.na(`Earnings-to-price`), `Earnings-to-price`, 
          !is.na(`Sales-to-price`), `Sales-to-price`, 
          !is.na(`Book-to-price`), `Book-to-price`) %>%
  mutate(Name_trimmed = factor(Name_trimmed, levels = Name_trimmed))

f1 = company_plot %>%
  ggplot(mapping = aes(x = Name_trimmed, y = 0)) + 
...
financial_plot = function(financials, variable, title = NULL, labels = waiver()) {
  ...
  financials %>%
    ggplot(mapping = aes(x = Name_trimmed, y = !!variable)) + 
...
name_width = 1.2 * max(stringWidth(company_plot$Name_trimmed) %>% convertUnit("inch") %>% as.numeric) / (stringWidth("Korea Shipbuilding &Offshore Engn") %>% convertUnit("inch") %>% as.numeric)
...
company_plot %<>% 
  ...
  mutate(Name_trimmed = Name_trimmed %>% str_replace_all(c("Taiwan Semiconductor" = "TSMC", 
                                                           "Pacific Biosciences of California" = "PacBio",
                                                           "Alibaba Group Holding - ADR" = "Alibaba",
...
These are a list of small-cap companies that interest me but need more research. Brief profile of each company is shown when available at [Wikipedia](https://en.wikipedia.org/wiki/Main_Page). For comprehensive financial data, see Section \@ref(comprehensive).

```{r}
make_wikitable = function(name) {
  wikitable = read_html(str_c("https://en.wikipedia.org/wiki/", name), which = 1, header = T, trim = T) %>% 
  ...
  missing = setdiff(c("Traded as", "Key people", "Services", "Industry", "Founders", "Services", "Subsidiaries"), names(wikitable))
  wikitable %<>% bind_cols(matrix(NA, nrow = 1, ncol = length(missing)) %>% set_colnames(missing) %>% as_tibble)
  
  wikitable %>%
    mutate(`Traded as` = str_replace_all(`Traded as`, "(.)(NYSE|NASDAQ|Russell)", "\\1<br>\\2"),
           across(c(`Key people`, Services), ~str_replace_all(., "\\)([A-Z])", ")<br>\\1")),
           across(c(Industry, Founders, Services, Subsidiaries), ~str_replace_all(., "([a-z])([A-Z])", "\\1<br>\\2"))) %>%
    t %>% 
    set_colnames("Val") %>%
...
```{r}
"Intellia_Therapeutics" %>% make_wikitable %>% display_wikitable("Intellia Therapeutics")
```
...
```{r}
wikitable = "Unity_Technologies" %>% make_wikitable
