...
company %<>% 
  ...
  mutate(PSR = Price / Revenue_per_share,
         ...
         `Relative_price` = get_relative_price(c(`Price/Target`, PEG, `Price/Graham`, `Price/DCF10`, `Marketcap/Payback8`)),
         Decision = case_when(BVPS < 0 ~ "Negative equity",
                              ...
                              sum(`Price/Target` < 1, PEG < 1, `Price/Graham` < 1, `Price/DCF10` < 1, `Marketcap/Payback8` < 1, na.rm = T) > 1 ~ "Maybe buy",
                              T ~ "Wait")) %>%
  ungroup %>%
