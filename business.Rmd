...
financials %<>% 
  left_join(company %>% select(Yahoo, Name)) %>%
  relocate(Name) %>%
  select(-Yahoo)
...
financials %<>% 
  rename(EBIT = Ebit) %>% 
  mutate(across(-c(Name, Date), make_readable)) %>%
...
company_plot %<>% 
  ...
  mutate(Name_trimmed = Name %>% 
           str_replace_all(c("Inc\\.*" = "",
                             "Ltd\\.*" = "",
                             "Mfg\\.*" = "",
                             "Common Stock" = "",
                             "(& )*Co(rporation|rp|\\.)*" = "",
...
relation_plot = function(financials, x, y, xtrans = "identity", xlab = NULL, ylab = NULL, xlabels = waiver(), ylabels = waiver()) {
  ...
  g = financials %>%
    ...
    scale_x_continuous(trans = xtrans, expand = c(0.25, 0), labels = xlabels) +
    scale_y_continuous(labels = ylabels) +
    scale_size_continuous(trans = "log10", guide = guide_legend(title = "Market\ncapitalization\n($Billion)")) +
    labs(x = xlab, y = ylab)
  
  g = ggplotGrob(g)
  
  label_indices = sapply(g$grobs[[15]][[1]][[1]]$grobs, function(i) "label" %in% names(i)) %>% which
  
  for (i in label_indices) {
    g$grobs[[15]][[1]][[1]]$grobs[[i]]$label = "A"
  }
...
f5 = relation_plot(company_plot, ROE, ROIC)

f6 = relation_plot(company_plot, DER, ROE, xtrans = "reverse", xlab = "Debt/Equity (reverse axis)")

f7 = relation_plot(company_plot, DER, ROIC, xtrans = "reverse", xlab = "Debt/Equity (reverse axis)")
```

```{r, fig.width = 7, fig.height = 40, fig.cap = "Scatterplot of key statistics"}
...
```{r, fig.width = 7, fig.height = (nrow(company_decision) + 6)/4, fig.cap = "Investment decision made by a fundamental analysis-based algorithm for the listed companies"}
company_decision %>%
