...
name_width = 1.2 * max(stri_width(company_plot$Name)) / stri_width("Korea Shipbuilding &Offshore Engn")
...
price_data %>% 
  ...
  facet_grid(rows = vars(Name), scales = "free", space = "free_x")
```
...
normalized_price_trend = function(price, industry) {
  ...
  g = price_subset %>%
    ...
    geom_vline(xintercept = base_date, linetype = "dashed") +
    ...
    annotation_custom(textGrob(str_c("Normalized to 100 as of ", base_date), 
                               y = unit(0.95, "npc"), 
                               hjust = ifelse(base_date < yearago, -0.03, 1.03), vjust = 1,
...
fig_height = (8 + get_legend_nrow(price_data, interesting_sectors))/2.5
```
