...
company_plot %<>% 
  ...
  mutate(Name_trimmed = clean_name(Name) %>% str_replace_all(c("Taiwan Semiconductor" = "TSMC", 
...
marketcap = company_plot %>% select(Name, Marketcap_USD)

marketcap %>% filter(Name == "Intellia Therapeutics") %>% pull(Marketcap_USD) %>% make_readable
```
...
wikitable %>% display_wikitable("Unity Technologies")
```
