...
financials_plot = financials %>% 
  ...
  group_by(Name) %>%
  mutate(across(c(`Total revenue`, `Net income`, `Total cash from operating activities`, `Total stockholder equity`),
                ~. / .[Date == min(Date)] * 100)) %>%
  ungroup
...
big_four_plot = function(.data, .variable, panelwidth) {
  ...
  .data %>%
    ggplot(mapping = aes(x = Date, y = !!.variable)) +
...
```{r, fig.cap = "Total revenue"}
big_four_plot(financials_plot, `Total revenue`, 6.5)
```

```{r, fig.cap = "Net income"}
big_four_plot(financials_plot, `Net income`, 6.5)
```

```{r, fig.cap = "Operating cash flow"}
big_four_plot(financials_plot, `Total cash from operating activities`, 6.5)
```

```{r, fig.cap = "Shareholder's equity"}
big_four_plot(financials_plot, `Total stockholder equity`, 6.5)
```
