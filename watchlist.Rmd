...
The trends of big 4 numbers of the last 3 to 4 years are shown below. All numbers are normalized to 100 (if positive) or -100 (if negative) as of the first year with available data.
...
big_four_plot = function(.data, .variable, panelwidth) {
  .data %<>% filter(Variable == .variable)
  
  get_padding = function(.data, .variable, panelwidth) {
    .data %>%
      count(Name) %>% 
      filter(n == 4) %>% 
      pull(Name) %>%
  ...
  .data %>%
    ...
    geom_text(data = .data %>% group_by(Name) %>% filter(Date == max(Date)),
              mapping = aes(x = Date + 10, label = Name), hjust = 0, size = 10/.pt) +
    scale_x_date(expand = c(0, 0, get_padding(.data, .variable, panelwidth), 0)) +
    labs(y = .variable)
}
```

```{r, fig.height = 7, fig.cap = "Total revenue"}
big_four_plot(financials_plot, "Net income", 6.4)
```

```{r, fig.height = 7, fig.cap = "Net income"}
big_four_plot(financials_plot, "Net income", 6.4)
```

```{r, fig.height = 7, fig.cap = "Operating cash flow"}
big_four_plot(financials_plot, "Total cash from operating activities", 6.4)
```

```{r, fig.height = 7, fig.cap = "Shareholder's equity"}
big_four_plot(financials_plot, "Total stockholder equity", 6.4)
```
