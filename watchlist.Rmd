...
my_datatable(financials, paging = T, pageLength = 8, scrollY = NULL, caption = NULL)
```
...
relation_plot = function(financials, x, y, xtrans = "identity", xlab = NULL, ylab = NULL, xlabels = waiver(), ylabels = waiver()) {
  ...
  padding = financials_filtered %>% 
    ...
    summarise(leftmargin = -min(leftbound) * diff(range(!!x)) / (2 * panelwidth + min(leftbound) - max(rightbound)), 
              rightmargin = (max(rightbound) - panelwidth)  * diff(range(!!x)) / (2 * panelwidth + min(leftbound) - max(rightbound)),
              minx = min(!!x),
              maxx = max(!!x)) %>%
    transmute(leftlim = minx - leftmargin,
              rightlim = maxx + rightmargin)
  
  g = financials_filtered %>%
    ...
    scale_x_continuous(trans = xtrans, expand = c(0, 0), labels = xlabels) +
    scale_size(trans = "log10", range = c(1, 6), guide = guide_legend(title = "Market cap\n($billion)")) +
    coord_cartesian(xlim = c(padding$leftlim, padding$rightlim)) +
    labs(x = xlab, y = ylab) +
