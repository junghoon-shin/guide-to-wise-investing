...
relation_plot = function(financials, x, y, xlab = NULL, ylab = NULL, xlabels = waiver(), ylabels = waiver()) {
  ...
  g = financials %>%
    ...
    scale_x_continuous(expand = c(0, 0), labels = xlabels) +
...
big_four_plot = function(.data, .variable, panelwidth, textsize) {
  ...
  xlabels = make_date(unique(year(seq(min(.data$Date), as_date(max(as.numeric(.data$Date)) * (1 + x_expansion)), by = 1)))[-1], 1, 1)
  ...
  g = .data %>%
    ...
    geom_text(data = .data %>% group_by(Name) %>% filter(Date == max(Date)),
              mapping = aes(x = Date + 10, label = Name), hjust = 0, size = textsize/.pt) +
...
```{r, fig.height = 9, fig.cap = "Total revenue"}
f1
```

```{r, fig.height = 9, fig.cap = "Net income"}
f2
```

```{r, fig.height = 9, fig.cap = "Operating cash flow"}
f3
```

```{r, fig.height = 9, fig.cap = "Shareholder's equity"}
f4
```
