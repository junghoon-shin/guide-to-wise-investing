...
index_plot(index_2yr, "MSCIEM", "MSCI Emerging Markets")
```

```{r, fig.height = 8/2.5, fig.cap = "Xtrackers Harvest CSI 300 China A-Shares ETF"}
index_plot(index_2yr, "CSI300", "Xtrackers Harvest CSI 300")
```

```{r, fig.height = 8/2.5, fig.cap = "iShares EURO STOXX 50 UCITS ETF (DE)"}
index_plot(index_2yr, "EUROSTOXX50", "iShares EURO STOXX 50")
```

```{r, fig.height = 8/2.5, fig.cap = "iShares EURO STOXX Select Dividend 30 UCITS ETF (DE)"}
index_plot(index_2yr, "EUROSTOXX30", "iShares EURO STOXX 30")
```
...
```{r, fig.height = 8/2.5, fig.cap = "South Korea/U.S. Foreign Exchange Rate"}
index_plot_fed(index_30yr, "USDKRW", "USD/KRW")
```
...
r = simplify2array(index_dailycor$r)
w = simplify2array(index_dailycor$w)
t = simplify2array(index_dailycor$w) %>% apply(1:2, function(x) sum(x, na.rm = T))

index_dailycor = apply(r * w, 1:2, function(x) sum(x, na.rm = T)) / t
...
r = simplify2array(index_monthlycor$r)
w = simplify2array(index_monthlycor$w)
t = simplify2array(index_monthlycor$w) %>% apply(1:2, function(x) sum(x, na.rm = T))

index_monthlycor = apply(r * w, 1:2, function(x) sum(x, na.rm = T)) / t
...
r = simplify2array(index_crosscor$r)
w = simplify2array(index_crosscor$w)
t = simplify2array(index_crosscor$w) %>% apply(1:2, function(x) sum(x, na.rm = T))

index_crosscor = apply(r * w, 1:2, function(x) sum(x, na.rm = T)) / t
```
