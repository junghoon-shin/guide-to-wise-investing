...
```{r}
Shiller = read_html("https://www.multpl.com/shiller-pe/table/by-month", which = 1, header = T) %>% rename(Shiller = "\n\nValue\n\n\n\nValue\n")
...
USvaluation = bind_rows(Shiller, Buffett) %>% filter(!is.na(price))
```
...
USvaluation %>% 
    ...
    coord_cartesian(xlim = c(as_date(fiftyyearsago) - 30, today())) +
    labs(x = "Date", y = "Buffett indicator")
```

(ref:shiller) [Cyclically Adjusted P/E (CAPE) Ratio for S&P 500](https://www.multpl.com/shiller-pe/table/by-year)
...
USvaluation %>% 
    ...
    scale_x_date(expand = c(0, 0)) +
    coord_cartesian(xlim = c(as_date(fiftyyearsago) - 30, today())) +
    labs(x = "Date", y = "Shiller P/E ratio")
```
