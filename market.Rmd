...
stock_exchange %<>% select(-c(Open, Close)) %>% slice(-1)

good = stock_exchange %>% filter(rowSums(is.na(.)) == 0)

good = stock_exchange %>% filter(!is.na(as.numeric(Year))) %>% select(-Rank)
bad = stock_exchange %>% filter(is.na(as.numeric(Year)))
names(bad) = c(names(bad)[-1], "dummy")
bad %<>% select(-dummy)
...
stock_exchange = bind_rows(good, bad) %>%
  ...
  mutate(across(everything(), str_trim),
         across(c(Year, Marketcap), ~str_replace_all(., ",", "") %>% as.numeric)) %>%
...
stock_exchange$`Country Name` %>% setdiff(marketcap$`Country Name`)

marketcap %>% arrange(desc(Marketcap))

stock_exchange

setdiff(marketcap$`Country Name`, gdp_ranking)
setdiff(gdp_ranking, marketcap$`Country Name`)

marketcap$`Country Name` %>% str_subset("Kingdom")

marketcap %>% filter(`Country Name` == "United Kingdom")
```
...
```{r}
top_country = marketcap %>% arrange(desc(Marketcap), desc(Year), desc(`Country Name`)) %>% pull(`Country Name`) %>% unique %>% .[1:5]
...
```{r, fig.height = (nrow(marketcap)/length(notNA_year) + 2)/4, fig.cap = str_c("Market capitalization of listed domestic companies across ", length(country_level), " major countries (proportion of the global market capitalization shown on the right side of each bar)")}
marketcap %>% 
