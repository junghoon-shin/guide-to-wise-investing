...
```{r, fig.height = (nrow(marketcap)/length(notNA_year) + 2)/4, fig.cap = str_c("Market capitalization of listed domestic companies across ", length(country_level), " major markets (proportions of the global market capitalization are shown on the right side of each bar)")}
marketcap %>% 
...
Others = country_mapping %>% 
  future_imap_dfr(~{
    tbl_list = read_html(str_c("https://www.gurufocus.com/global-market-valuation.php?country=", .x), header = T)
    tbl_list[[which(map_lgl(tbl_list, ~{"Valuation" %in% names(.)}))]] %>% mutate(Country = .y)
  })
