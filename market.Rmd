...
stock_exchange = bind_rows(good, bad) %>%
  ...
  mutate(across(c(Year, Marketcap), ~str_replace_all(., ",", "") %>% as.numeric),
         `Country Name` = case_when(str_detect(`Country Name`, "United Kingdom") & str_detect(`Country Name`, "Italy") ~ "United Kingdom/Italy",
                                    T ~ `Country Name`),
...
stock_exchange %<>% filter(!`Country Name` %in% c("European Union"))
...
marketcap %>% count(`Country Name`) %>% arrange(n)
```
...
```{r, fig.height = (nrow(marketcap)/length(notNA_year) + 2)/4, fig.cap = str_c("Market capitalization of listed domestic companies across ", length(country_level), " major markets (proportion of the global market capitalization shown on the right side of each bar)")}
marketcap %>% 
