...
legend_label_width = c(country_level_rev[1:12], "Other countries") %>% stringWidth %>% convertUnit("inch") %>% max %>% as.numeric %>% multiply_by(0.8)
...
combined %>%
  ...
  ggplot(mapping = aes(x = GDP/1e12, y = Marketcap/1e12)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
  geom_text(mapping = aes(label = `Country Name`), hjust = 0.5, vjust = 0.5, size = 9/.pt, family = "Lato") +
  scale_x_continuous(trans = "log10") +
  scale_y_continuous(trans = "log10") +
  labs(x = "GDP ($Trillion)", y = "Market capitalization ($Trillion)") +
  facet_grid(rows = vars(Year), scales = "free")
