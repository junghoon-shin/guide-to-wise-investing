...
```{r buffett, fig.width = 7, fig.height = 5 * length(unique(combined$Year)), fig.cap = str_c("Market capitalization relative to GDP across ", length(unique(combined$`Country Name`)), " major countries")}
ggarrange(g, g_legend, widths = c(4, 1))
```

(ref:buffett-text) Same data as in Figure \@ref(fig:buffett)

```{r, fig.width = 7, fig.height = 6.5 * length(unique(combined$Year)), fig.cap = "(ref:buffett-text)"}
combined %>%
  ggplot(mapping = aes(x = GDP/1e12, y = Marketcap/1e12)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
