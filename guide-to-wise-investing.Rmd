...
description: "Investment guidebook containing business financials, industry and market environment, and global economy"
---
...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, out.width = "100%", out.height = "100%", fig.width = 7, fig.height = 5)
...
thisyear = today() %>% format("%Y") %>% as.numeric
...
tibble(Moat = c("Strong brand", 
                ...
                "Secret<br>(Patent)", 
                "Price"),
       Example = c("![](images/Coca_Cola.png){height=80px} ![](images/Starbucks.png){height=80px} ![](images/Hermes.png){height=80px}",
                   "![](images/Apple.png){height=80px} ![](images/Facebook.png){height=80px} ![](images/Kakao.png){height=80px} ![](images/Twitter.png){height=80px} ![](images/TikTok.png){height=80px}",
...
glossary = tribble(~Concept, ~Meaning,
        ...
        "Payback8", "8-year payback time buy price [@town2018]",
...
big_four_plot = function(.data, .variable, panelwidth, textsize) {
  ...
  g = .data %>%
    ...
    facet_grid_sc(rows = vars(group),
                  scales = list(x = "fixed",
                                y = list(High = scale_y_continuous(expand = c(0.1, 0)),
                                         Main = scale_y_continuous(expand = c(0.02, 0)),
                                         Low = scale_y_continuous(expand = c(0.1, 0)))),
                  labeller = labeller(group = c(High = "Upper outlier", Main = "Majority", Low = "Lower outlier")))
...
### CymaBay Therapeutics (`r marketcap %>% filter(Name_trimmed == "CymaBay Therapeutics") %>% pull(Marketcap_USD) %>% make_readable %>% str_c(" $")`)
...
```{r, fig.height = 2.5 * length(unique(price_data$Name)), fig.cap = "Adjusted closing stock price over the last 2 years"}
price_data %<>%
...
g$layout$clip[str_detect(g$layout$name, "panel")] = "off"
...
```{r}
industry2 = read_html("http://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/mgnroc.html", which = 1, header = T)

names(industry2) %<>% str_replace_all("\\s+", " ")
...
index_plot(index_2yr %>% filter(name == "MSCIEM"), "MSCI Emerging Markets", from = as_date(twoyearsago), ribbon = T)
```
...
index_plot(index_2yr %>% filter(name == "Copper"), "Copper price", from = as_date(twoyearsago), ribbon = T)
```

```{r, fig.height = 8/2.5, fig.cap = "Crude Oil Mar 21"}
index_plot(index_2yr %>% filter(name == "Oil"), "Crude oil price", from = as_date(twoyearsago), ribbon = T)
```

```{r, fig.height = 8/2.5, fig.cap = "U.S. Treasury Bond Futures"}
index_plot(index_2yr %>% filter(name == "Treasuryfutures"), "U.S. T-note futures", from = as_date(twoyearsago), ribbon = T)
```

```{r, fig.height = 8/2.5, fig.cap = "iShares Aaa - A Rated Corporate Bond ETF"}
index_plot(index_2yr %>% filter(name == "AAA"), "iShares AAA", from = as_date(twoyearsago), ribbon = T)
```

```{r, fig.height = 8/2.5, fig.cap = "Treasury Yield 10 Years"}
index_plot(index_2yr %>% filter(name == "Treasuryyield"), "U.S. T-note yield", from = as_date(twoyearsago), ribbon = T)
```
...
```{r, fig.height = 8/2.5, fig.cap = "U.S. Treasury Bond Futures"}
index_plot(index_30yr %>% filter(name == "USBondFutures"), "U.S. T-note futures", from = as_date(thirtyyearsago) - 30)
```

```{r, fig.height = 8/2.5, fig.cap = "Long-Term Government Bond Yields: 10-year: Main (Including Benchmark) for the United States"}
index_plot(index_30yr %>% filter(name == "USBondYield"), "U.S. T-note yield", from = as_date(thirtyyearsago) - 30)
```

```{r, fig.height = 8/2.5, fig.cap = "Moody's Seasoned AAA Corporate Bond Yield"}
index_plot(index_30yr %>% filter(name == "AAA"), "AAA corporate bond yield", from = as_date(thirtyyearsago) - 30)
```
## Index correlation
...
## Market capitalization^[`r str_c('Data are from [World Bank](https://data.worldbank.org/indicator/CM.MKT.LCAP.CD?view=chart). Most recent 3 years for which top 3 countries\' data are available are shown.')`]
...
stock_exchange = read_html("https://en.wikipedia.org/wiki/List_of_stock_exchanges", which = 1, header = T, trim = T)
