...
f1 = allocation %>%
  ...
  geom_text(mapping = aes(x = 1.3, y = current_y, label = make_percent(`Current proportion`, 100)), hjust = 0.5, vjust = 0.5) +
  scale_y_continuous(breaks = pretty_breaks(), labels = function(x) {percent(rev(x))}) +
  scale_fill_manual(values = fill_color, guide = guide_legend(title = NULL)) +
  coord_polar(theta = "y", direction = -1) +
...
f2 = allocation %>%
  ...
  geom_col(mapping = aes(fill = category), position = "fill", color = "black") +
  geom_text(mapping = aes(x = 1.3, y = target_y, label = make_percent(`Target proportion`, 100)), hjust = 0.5, vjust = 0.5) +
  scale_y_continuous(breaks = pretty_breaks(), labels = function(x) {percent(rev(x))}) +
  scale_fill_manual(values = fill_color, guide = guide_legend(title = NULL)) +
  coord_polar(theta = "y", direction = -1) +
