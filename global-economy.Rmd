...
gdp %<>% mutate(`Country Name` = str_replace_all(`Country Name`, c("Russian Federation" = "Russia",
  ...
  filter(!is.na(GDP) & Year >= (thisyear - 30) & `Country Name` %in% country_ranking) %>%
...
gdp_growth %<>% mutate(`Country Name` = str_replace_all(`Country Name`, c("Russian Federation" = "Russia",
  ...
  filter(!is.na(`GDP growth`) & Year >= (thisyear - 30) & `Country Name` %in% country_ranking) %>%
  mutate(`Country Name` = factor(`Country Name`, levels = country_ranking))
```
...
inflation %<>% mutate(`Country Name` = str_replace_all(`Country Name`, c("Russian Federation" = "Russia",
  ...
  filter(!is.na(Inflation) & Year >= (thisyear - 30) & `Country Name` %in% country_ranking) %>%
  mutate(`Country Name` = factor(`Country Name`, levels = country_ranking))
...
inflation %>% 
  ...
  coord_cartesian(ylim = c(min(inflation$Inflation), 0.25)) +
...
```{r}
US = read_csv(str_c("https://fred.stlouisfed.org/graph/fredgraph.csv?id=", "USALORSGPNOSTSAM", "&cosd=", fiveyearsago, "&coed=", today,
