...
gdp %<>% mutate(`Country Name` = str_replace_all(`Country Name`, c("Russian Federation" = "Russia",
  ...
  pivot_longer(-`Country Name`, names_to = "Year", values_to = "GDP (USD)") %>%
  mutate(Year = as.numeric(Year)) %>%
  filter(!is.na(`GDP (USD)`) & Year >= (thisyear - 30) & `Country Name` %in% country_ranking) %>%
...
g = gdp %>% 
  ggplot(mapping = aes(x = Year, y = `GDP (USD)`)) +
  geom_line(mapping = aes(color = Country)) +
  scale_x_continuous(expand = c(0, 0), breaks = pretty_breaks()) +
  scale_y_continuous(trans = "log10") +
  scale_color_manual(values = qual_col_vector[-6][1:length(unique(gdp$Country))], guide = guide_legend(title = NULL)) +
...
g = gdp_growth %>% 
  ...
  scale_color_manual(values = qual_col_vector[-6][1:length(unique(gdp_growth$Country))], guide = guide_legend(title = NULL)) +
  theme(legend.key = element_rect(fill = "transparent"))
...
g = inflation %>% 
  ...
  scale_x_continuous(expand = c(0, 0), breaks = pretty_breaks()) +
  scale_y_continuous(labels = percent) +
  scale_color_manual(values = qual_col_vector[-6][1:length(unique(inflation$Country))], guide = guide_legend(title = NULL)) +
...
```{r, fig.width = 7, fig.height = 4, fig.cap = "OECD Composite Leading Indicators (CLIs) normalized for US, China, Euro, and South Korea over the last 5 years"}
CLI %>% 
