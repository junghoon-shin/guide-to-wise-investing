...
CLI = list(US, China, Euro, Korea) %>%
  ...
  mutate(Region = relabelCLI(Region) %>% factor(levels = c("US", "China", "Euro", "Korea"), labels = c("US", "China", "Euro", "Korea")))
```
...
CLI %>% 
  ...
  scale_color_manual(values = line_color[1:length(unique(CLI$Region))], labels = c("US", "China", "Euro", "Korea"), guide = guide_legend(title = NULL)) +
  scale_fill_manual(values = line_color[1:length(unique(CLI$Region))], labels = c("US", "China", "Euro", "Korea"), guide = guide_legend(title = NULL)) +
  labs(x = "Year", y = "CLI")
```
