...
g = gdp %>% 
  ...
  scale_color_manual(values = qual_col_vector[1:length(unique(gdp$Country))], guide = guide_legend(title = NULL)) +
...
g = gdp_growth %>% 
  ...
  scale_color_manual(values = qual_col_vector[1:length(unique(gdp_growth$Country))], guide = guide_legend(title = NULL)) +
  theme(legend.key = element_rect(fill = "transparent"))
...
g = inflation %>% 
  ...
  scale_x_continuous(expand = c(0, 0), breaks = pretty_breaks()) +
  scale_y_continuous(labels = percent) +
  scale_color_manual(values = qual_col_vector[1:length(unique(inflation$Country))], guide = guide_legend(title = NULL)) +
