...
price_data %<>% 
  ...
  filter(symbol %in% sufficient_data)
```
...
price_today = price_data %>%
  filter(as_date(date) %in% (as_date(date) - 0:5)) %>% 
  group_by(symbol) %>% 
  arrange(desc(date)) %>% 
  slice(1) %>% 
  ungroup %>% 
...
price_data %>% 
  ...
  geom_hline(data = price_today, mapping = aes(yintercept = adjusted), size = rel(0.5), color = line_color[2]) +
  scale_x_date(expand = c(0, 0)) +
...
h = price_cor %>% Heatmap(name = "Pearson's r",
                        width = unit(5.25, "inch"), height = unit(0.11 * nrow(price_cor), "inch"),
...
```{r, fig.width = 7, fig.height = h_height + 0.2, fig.cap = "Pairwise correlation between adjusted closing prices over 2 years of my watchlist"}
h
```