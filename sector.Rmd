...
```{r, fig.width = 7, fig.height = (notNA_per + 3)/4, fig.cap = "Current, trailing, and forward PE ratio by industry sector (note the x-axis is in log scale)"}
industry_per %>%
  ggplot(mapping = aes(x = `Industry Name`, y = `PE ratio`)) +
  geom_col(fill = brewer_pal("qual", 4)(2)[2], color = "black", width = 0.7) +
  geom_text(mapping = aes(y = `PE ratio` + 50, label = round(`PE ratio`)), hjust = 0, size = 9 / .pt, family = "Lato") +
